...
class TestClassicalPawn(BaseTestPieces):
    ...
    def test_check_move(self):
        ...
        self.assertIsNotEmpty(self.piece1.check_piece_valid_move(Position("b4")))
        self.assertIsEmpty(self.piece1.check_piece_valid_move(Position("a3")))
        self.assertIsEmpty(self.piece1.check_piece_valid_move(Position("c3")))
        self.assertIsEmpty(self.piece1.check_piece_valid_move(Position("b5")))
        self.assertIsEmpty(self.piece1.check_piece_valid_move(Position("b1")))

        position = Position("e8")
        promotion = self.piece2.check_piece_valid_move(position)
        ...
        self.assertIsEmpty(self.piece2.check_piece_valid_move(Position("d8")))
        self.assertIsEmpty(self.piece2.check_piece_valid_move(Position("f8")))
    ...
    def test_capture(self):
        ...
        self.assertIn(Promotion(self.piece2, position, self.piece2.valid_pieces_type[0]),
                      self.piece2.get_piece_valid_moves())

    def test_piece_in_front(self):
        position = Position("b3")
        self.board.add_piece(self.Pawn(self.board, self.other_player, position))

        self.assertIsEmpty(self.piece1.check_piece_valid_move(position))
        self.assertIsEmpty(self.piece1.get_piece_valid_moves())
        ...
        self.assertIsEmpty(self.piece1.check_piece_valid_move(position))
        self.assertIsNotEmpty(self.piece1.check_piece_valid_move(Position("b3")))
        self.assertEqual([BasicMovement(self.piece1, Position("b3"))], self.piece1.get_piece_valid_moves())

        position = Position("e8")
        self.board.add_piece(self.Pawn(self.board, self.other_player, position))

        self.assertIsEmpty(self.piece2.check_piece_valid_move(position))
        self.assertIsEmpty(self.piece2.get_piece_valid_moves())
    ...
    def test_en_passant(self):
        ...
        self.game_mode.current_player_turn = next(self.game_mode.order)
        self.game_mode.force_move(other_piece.check_piece_valid_move(Position("f5"))[0])

        self.assertIsNotEmpty(main_piece.check_piece_valid_move(Position("f6")))
        self.assertIn(EnPassant(main_piece, Position("f6"), other_piece), main_piece.get_piece_valid_moves())
        self.assertIsEmpty(main_piece.check_piece_valid_move(Position("d6")))

        other_piece.n_moves = 2
        self.assertIsEmpty(main_piece.check_piece_valid_move(Position("f6")))
